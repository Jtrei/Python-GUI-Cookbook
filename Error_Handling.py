...
def account_creation_error(username, password, password_confirmation, user_folder):
    if len(username) < 4:
        ...
        text_response = 'Password and confirmatory Password do not match, please try again.'
    elif os.path.exists(user_folder):
        text_response = f'Username {username} already exists, please choose new username.'
    else:
