...
folder_path_to_configuration_directory = os.path.join(
    folder_path_to_data_folder, "Configuration"
)  # .\Data\Configuration
folder_path_to_database = os.path.join(
...
class StartupGUI(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        recipe_sub_menu.add_command(
            label="Add recipe", command=lambda: self.menu_check(Manual_Recipe_Adder)
        )
        recipe_sub_menu.add_command(
            label="Add recipe from website",
            command=lambda: self.menu_check(Online_Recipe_Adder),
        )
        recipe_sub_menu.add_command(
            label="Browse recipes", command=lambda: self.menu_check(Recipe_Browser)
        )
        recipe_sub_menu.add_command(
            label="Import Recipes (from text file)",
            command=lambda: self.menu_check(Import_Recipes_From_Text),
        )
        recipe_sub_menu.add_command(
            label="Export Recipes (to PDF)",
            command=lambda: self.menu_check(Export_Recipe_To_PDF),
        )
        ...
        gui_frames = [
            ...
            Online_Recipe_Adder,
            Recipe_Browser,
            Import_Recipes_From_Text,
            Export_Recipe_To_PDF,
            Edit_Recipe,
            Edit_Settings,
            Edit_View,
        ]
        for Frame in gui_frames:
            ...
            self.frames[
                Frame
            ] = frame  # self.frames[LoginPage] = LoginPage[container, self]
...
class Main_Page(tk.Frame):
    def __init__(self, parent, controller):
        ...
        try:
            ...
            self.welcome_label = tk.Label(
                self,
                text="Welcome to my cooking app. It is a WIP. All user's photos will appear here once you add some recipes",
            )
            self.welcome_label.grid(row=0, column=0, columnspan=4, sticky=tk.W)
...
def main_program():
    ...
    width, height = int(main_app.winfo_screenwidth() // 1.5), int(
        main_app.winfo_screenheight() // 1.5
    )
