...
website_ingredient_html_info = {
    "tasteofhome": [
        "ul",
        "recipe-ingredients__list recipe-ingredients__collection splitColumns",
        "li",
    ],
}
...
class Scraper:
    ...
    def scrape_recipe(self):
        for k, v in website_name.items():
            if k in self.url:
                # Title
                parent_heading, class_name = (
                    website_title_html_info[k][0],
                    website_title_html_info[k][1],
                )
                for element in self.soup.find_all(parent_heading, class_=class_name):
                    self.title
                # Ingredients
                parent_heading, class_name, child_heading = (
                    website_ingredient_html_info[k][0],
                    website_ingredient_html_info[k][1],
                    website_ingredient_html_info[k][2],
                )
                for element in self.soup.find_all(parent_heading, class_=class_name):
...
def add_recipe_scraping(
    ...
    cur.execute(
        f"""INSERT INTO {username}_Recipes VALUES 
...
def MainExtraction():
    collection_of_websites = {
        "tasteofhome": taste_of_home(Web_recipe),
